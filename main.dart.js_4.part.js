((a,b)=>{a[b]=a[b]||{}})(self,"$__dart_deferred_initializers__")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,E,F,B={
boA(d){return new B.v4(d,null)},
v4:function v4(d,e){this.c=d
this.a=e},
ahp:function ahp(d){this.a=d},
ahq:function ahq(){},
ahn:function ahn(){},
aho:function aho(){},
ahm:function ahm(){}},D
A=c[0]
C=c[2]
E=c[7]
F=c[8]
B=a.updateHolder(c[4],B)
D=c[9]
B.v4.prototype={
A(d){var x,w=null,v=A.q(d),u=C.b.iX(F.lQ,new B.ahp(this),new B.ahq()),t=u.c,s=A.cK(w,w,A.n(t,w,w,w,w,w,w,w)),r=v.ax.b,q=r.X(0.12),p=A.av(999),o=v.ok,n=o.ax
r=n==null?w:n.dq(r,C.ag)
p=A.aj(w,A.n(t+" \xb7 "+u.r+" Min",w,w,w,r,w,w,w),C.j,w,w,new A.aw(q,w,w,p,w,w,C.v),w,w,w,C.cZ,w,w,w)
q=A.n(u.b,w,w,w,o.f,w,w,w)
o=o.y
t=o==null?w:o.aGm(C.h3)
r=y.e
t=A.b([p,C.I,q,C.u,A.n(u.d,w,w,w,t,w,w,w),C.w7],r)
for(q=this.aBq(u.e),p=q.length,x=0;x<q.length;q.length===p||(0,A.K)(q),++x)C.b.K(t,A.b([this.azi(v,q[x]),C.I],r))
return A.cz(s,w,A.dH(t,C.F,w,C.M,!1),w,w,w)},
aBq(d){var x=C.c.uo(d,A.cN("\\n\\s*\\n",!0,!1))
x=new A.W(x,new B.ahn(),A.Z(x).i("W<1,i>")).xX(0,new B.aho())
x=A.S(x,x.$ti.i("C.E"))
return x},
azi(d,e){var x,w,v,u,t,s=null,r=C.b.ga8(e.split("\n"))
if(r===r.toUpperCase()&&r.length<80&&!C.c.iz(r,".")&&e.split("\n").length===1){x=d.ok.w
return A.n(e,s,s,s,x==null?s:x.dq(d.ax.b,C.c4),s,s,s)}if(C.b.dF(A.b(e.split("\n"),y.h),new B.ahm())){x=A.b([],y.e)
for(w=e.split("\n"),v=w.length,u=d.ok.z,t=0;t<v;++t)x.push(new A.a4(C.po,A.n(w[t],s,s,s,u,s,s,s),s))
return A.aA(x,C.p,C.e,C.h)}return A.n(e,s,s,s,d.ok.z,s,s,s)}}
var z=a.updateTypes(["D(dY)","dY()"])
B.ahp.prototype={
$1(d){return d.a===this.a.c},
$S:z+0}
B.ahq.prototype={
$0(){return D.QC},
$S:z+1}
B.ahn.prototype={
$1(d){return C.c.bc(d)},
$S:34}
B.aho.prototype={
$1(d){return d.length!==0},
$S:22}
B.ahm.prototype={
$1(d){var x
if(!C.c.bB(d,"- ")){x=A.cN("^\\d+\\.\\s",!0,!1)
x=x.b.test(d)}else x=!0
return x},
$S:22};(function inheritance(){var x=a.inherit,w=a.inheritMany
x(B.v4,A.P)
w(A.l3,[B.ahp,B.ahn,B.aho,B.ahm])
x(B.ahq,A.rb)})()
A.aem(b.typeUniverse,JSON.parse('{"v4":{"P":[],"c":[]}}'))
var y={h:A.a0("B<i>"),e:A.a0("B<c>")};(function constants(){D.QC=new E.dY("unknown","Artikel nicht gefunden","Unbekannt","","Dieser Artikel existiert nicht (mehr).",3)})()};
(a=>{a["HHpCv6eBmgG6/XzRJ55wHcLYxR8="]=a.current})($__dart_deferred_initializers__);